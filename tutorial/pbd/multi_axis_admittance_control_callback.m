...
persistent pos_seq;
persistent n previous_updated_sensor_V_sensor
persistent static_pose;


vel_msg = handles.vel_msg;
vel_pub = handles.vel_pub;

% define M B K matrix in target frame
M = [0.05*eye(3) zeros(3,3);zeros(3,3) 0.1*eye(3)];
B = [0.2*eye(3) zeros(3,3);zeros(3,3) 0.2*eye(3)];
...
if isempty(previous_updated_sensor_V_sensor)
    previous_updated_sensor_V_sensor = zeros(6,1);
end

if isempty(static_pose)
    base_trans_sensor = ati_pose_data(1:3);
    base_rot_sensor = quat2rotm(ati_pose_data(4:7)');
    static_pose = RpToTrans(base_rot_sensor, base_trans_sensor);
end
...
if ~isempty(pre_pose_data)
    ...
    % using system vel interface
    base_V_tool = current_vel_data;
    base_V_tool([1:3 4:6]) = base_V_tool([4:6 1:3]); % change to the form of [omega v]'
    tool_V_tool = base_rot6_tool' * base_V_tool;
    ...
    if norm(contact_force) > 0.001
        updated_sensor_V_sensor = zeros(6,1);
        contact_force
        dV = M\(contact_force - K*sensor_S_theta - B*sensor_V_sensor);
        % compute spatial twist
        updated_sensor_V_sensor(1:3) = (0.8*dV(1:3) + 0.2*previous_updated_sensor_V_sensor(1:3))/(loop_rate_hz);
        updated_sensor_V_sensor(4:6) = (0.5*dV(4:6)/(loop_rate_hz) + 0.5*previous_updated_sensor_V_sensor(4:6))
        previous_updated_sensor_V_sensor = updated_sensor_V_sensor;
    else
        updated_sensor_V_sensor = zeros(6,1);
        previous_updated_sensor_V_sensor = updated_sensor_V_sensor;
    end
    ...
    tool_V_tool = Adjoint(inv(sensor_T_tool)) * updated_sensor_V_sensor;
    
    base_V_tool = base_rot6_tool * tool_V_tool;
    
    vel_msg.Angular.X = base_V_tool(1);
    vel_msg.Angular.Y = base_V_tool(2);
    vel_msg.Angular.Z = base_V_tool(3);
    vel_msg.Linear.X = base_V_tool(4);
    vel_msg.Linear.Y = base_V_tool(5);
    vel_msg.Linear.Z = base_V_tool(6);
